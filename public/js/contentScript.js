...
;function onClickElement(event) {
    ...
    var elo = new ElementsLocatorObj(location.href,null,new ElementLocator(getXPath(t),getCssPath(t),t.tagName,t.id,t.className,t.innerText,t.baseURI,getElementAttributes(t)));
...
;function getElementAttributes(element) {
    ...
    for (var index = 0; index < elementAttributeNames.length; index++) {
        if (elementAttributeNames[index] == 'class' && element.getAttribute(elementAttributeNames[index]) == '') {
            continue
        }
...
function unMarkAllMarks() {
    ...
    var markElementsTemp = document.querySelectorAll('*[class*="marked-element-temp"]');
    for (var index = 0; index < markElementsTemp.length; index++) {
        unMarkElement(markElementsTemp[index], !0)
    }
    var markElementsSuccess = document.querySelectorAll('*[class*="mark-successfully"]');
    ...
    if (eventPageMessage.elementLocator) {
        if (eventPageMessage.elementLocator.cssSelector) {
            ...
            for (var i = 0; i < eventPageMessage.elementLocator.length; i++) {
                markElement(document.querySelector(eventPageMessage.elementLocator[i].cssSelector))
            }
        }
    } else {
        unMarkAllMarks()
...
function getElementsCheckResults(checkElements) {
    for (var index = 0; index < checkElements.length; index++) {
        ...
        var elementInPage = document.querySelectorAll(elementCheck.elementLocator.cssSelector);
        elementCheck.isExist = elementInPage.length > 0 ? !0 : !1;
        elementCheck.cssSelectorIsDuplicated = elementInPage.length > 1 ? !0 : !1;
        var elementsInPageWithXpath = getElementsByXPath(elementCheck.elementLocator.xpath, document);
        elementCheck.xpathIsDuplicated = elementsInPageWithXpath.length > 1 ? !0 : !1;
...
function showElementOnPage(eventPageMessage) {
    ...
    if (currentElement) {
        ...
        var element = document.querySelector(eventPageMessage.elementLocator.cssSelector);
        if (element) {
...
function sendElementsLocatorObj() {
    if (document.visibilityState == 'visible') {
        chrome.runtime.sendMessage(new ElementsLocatorObj(location.href,null,null))
    }
}
